...
## A sheet older than the tabs

`Client`, `Templates` and `Fields` were added on **2026-08-12**. A Proposal Config sheet created
before that date has only `Chapters`, `Content` and `Rules`, and the pipeline treats it as valid,
because every one of the three is optional by design — a client onboarded before they existed keeps
working untouched. That is the right behaviour and it is also why nothing tells you the tabs are
missing.

What each absent tab costs:

| Missing tab | What happens | How bad |
|---|---|---|
| `Client` | `document_version` falls back to `1.0`, the version table's author column comes out blank, and the Drive folder ids come from the Notion row instead | Cosmetic, unless the folder ids are only in the sheet |
| `Templates` | Template selection falls back to the registry's `template_id_es` / `template_id_en` — one document per language, no variants | Fine until the client needs a second document |
| `Fields` | `campos` is **empty**, so every `{campos.*}` tag in the template prints the literal word **`undefined`** | **Reaches the customer.** This is the one to fix before sending anything |

The third row is not a warning you will see anywhere: the totality rule that keeps every other key
present-but-empty cannot cover `campos`, because with no `Fields` tab there is nothing to enumerate.
The template asks for a key, the context has none, and docxtemplater prints the word.

### Bringing one up to date

The repo already holds each client's six tabs as CSVs. Add the missing ones as new tabs with
**exactly these names** — `Client`, `Templates`, `Fields` — and paste the matching CSV in, header row
included:

```
seed/<client_id>/proposal-config/client.csv
seed/<client_id>/proposal-config/templates.csv
seed/<client_id>/proposal-config/fields.csv
```

`Templates` may stay empty apart from its header; the other two should not. Then check what the
pipeline will actually see:

```bash
node scripts/client-docs.js <client_id>   # every {campos.*} tag their template may use
node scripts/check-template.js <their-template.docx> <client_id>
```

`check-template.js` is the one that catches this specific failure: it reads every tag in the `.docx`
and fails on any `{campos.*}` the `Fields` tab does not declare. Run it after adding the tab and
after every edit in Word.

> **`demo_client` is the urgent case.** Its template goes to strangers with nobody reading it in
> between, and the demo templates generated by `npm run templates` carry five `{campos.*}` tags. A
> demo sheet without a `Fields` tab puts `undefined` on a prospect's cover page.
