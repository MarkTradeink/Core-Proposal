...
| `send_mode` | Select: `send` / `draft` (empty → `send`) | Delivery switch. `send` replies to the reseller for real; `draft` stops at a Gmail draft and sends nothing. The per-client rollback — set it to `draft` to take one client out of live sending without touching the workflows. **Not consulted for `demo_client` or for anything that arrived through `demo@cifral.io`:** the demo tenant's mode is decided in code by `DEMO_SEND_MODE` (currently `send`), because a default nobody chose must not decide what a public address does ([`docs/DEMO-INTAKE.md`](DEMO-INTAKE.md) §3). Whatever this cell says on `demo_client`'s row has no effect. |
